...
async function run() {
  ...
  for (const product of ready) {
    ...
    if (result.success === false) {
      console.error(`[marketing-agent] Gemini failed for ${product.id}: ${result.error} — ${result.message}`);
      ...
      if (products[idx0].aiFailureCount > 3) {
        ...
        console.error(`[marketing-agent] Product ${product.id} exceeded failure limit, flagged for human review`);
      }
      continue;
    }
    ...
    try {
      ...
      console.error(`[marketing-agent] JSON parse error for ${product.id}: ${err.message}`);
      const idx0 = products.findIndex(p => p.id === product.id);
      products[idx0].aiFailureCount = (products[idx0].aiFailureCount || 0) + 1;
